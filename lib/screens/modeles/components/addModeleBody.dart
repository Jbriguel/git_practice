...
class AddModeleBody extends StatefulWidget {
  ...
  @override
  State<AddModeleBody> createState() => _AddModeleBodyState();
}

class _AddModeleBodyState extends State<AddModeleBody> {
  File? imgFile;
  String? image;
  ...
  void saveModele(MyInheritedWidgetState state) async {
    ...
    _modeleRepository
        .saveModele(modele, state.proprieties_selected)
        .then((retour) {
      if (retour == true) {
        clearFields();
        setState(() {});
    ...
    // if (success) {
    //   Navigator.pop(context, true);
    //   //  widget.onSave(); // Notifier le parent du succès
    // } else {}
